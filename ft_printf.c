...
/*   Updated: 2013/12/12 18:57:15 by ael-kadh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_printf(const char *str, ...)
{
	...
	while (str[i] != '\0')
	{
		if (str[i] == '%')
		{
			if (str[i + 1] == '#' || str[i + 1] == '-' || (str[i + 1] == '.' && (str[i + 2] == 'd' || str[i + 2] == 'i')))
			{
				length--;
				i++;
			}
...
int		condition(const char *str, int *i, va_list ap)
{
	...
	else if (str[*i + 1] == 'd' || str[*i + 1] == 'i' || ((str[*i + 1] == ' ' || str[*i + 1] == '+') && (str[*i + 2] == 'd' || str[*i + 2] == 'i')))
	{
		x = va_arg(ap, int);
